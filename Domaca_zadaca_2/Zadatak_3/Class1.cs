...
namespace Zadatak_4
{
    public class HomeworkLinqQueries
    {
        ...
        {
            return intArray.Distinct().Select(i => $"Broj {i} ponavlja se {intArray.Count(s => s == i)} puta").ToArray();
        }
        ...
        public static Student[] Linq2_3(University[] universityArray)
        {
            return universityArray
                ...
                .SelectMany(stu => stu.Students).ToArray();
        }
        public static Student[] Linq2_4(University[] universityArray)
        {
            return universityArray
                .Select(s => new University {Name = s.Name, Students = s.Students.Distinct().ToArray()}).Where(s =>
                    ...
                    s.Students.Count() == s.Students.Count(i => i.Gender == Gender.Female)).SelectMany(stu => stu.Students).ToArray();
        }
